...
const POMODORO_STRINGS = {
  ...
  openBoardOnPomodoroEndDesc: { ja: 'ONにすると、ポモドーロが終了したときにこのウィジェットが属するボードを自動で開きます。', en: 'If ON, the board containing this widget will open automatically when Pomodoro ends.' },
  autoStartNextPomodoroSession: { ja: 'ポモドーロ終了時に自動で次のセッションを開始', en: 'Automatically start the next session when Pomodoro ends' },
  autoStartNextPomodoroSessionDesc: { ja: 'ONにすると、ポモドーロが終了したときに自動で次のセッションを開始します。', en: 'If ON, the next session will start automatically when Pomodoro ends.' },
